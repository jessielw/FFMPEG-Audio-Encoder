...
root = Tk()
root.title("FFMPEG Audio Encoder Alpha v0.7")
...
def startaudiojob():
    if shell.get() == "powershell.exe":
        ...
        subprocess.Popen(shell.get() + " ffmpeg -i " + VideoInputQuoted + " " + acodec_stream_choices[acodec_stream.get()] + " -c:a " + encoder_dropdownmenu_choices[encoder.get()] + " -b:a " + acodec_bitrate.get() + " -ac " + acodec_channel_choices[acodec_channel.get()] + " -sn -vn " + VideoOutputQuoted)
    elif shell.get() == "cmd":
        VideoInputQuoted = '"' + VideoInput + '"'
        VideoOutputQuoted = '"' + VideoOutput + '"'
        subprocess.Popen(shell.get() + " /c ffmpeg -i " + VideoInputQuoted + " " + acodec_stream_choices[acodec_stream.get()] + " -c:a " + encoder_dropdownmenu_choices[encoder.get()] + " -b:a " + acodec_bitrate.get() + " -ac " + acodec_channel_choices[acodec_channel.get()] + " -sn -vn " + VideoOutputQuoted)
