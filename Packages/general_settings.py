...
def open_general_settings():  # General Settings Window
    ...
    def general_settings_exit_function():  # Exit function when hitting the 'X' button
        ...
        if (
            general_parser["save_window_locations"]["general settings"] == "yes"
        ):  # If auto save position on
            try:
                if (
                    general_parser["save_window_locations"]["general settings position"]
    ...
    if (
        config_parser["save_window_locations"]["general settings position"] == ""
        or config_parser["save_window_locations"]["general settings"] == "no"
    ):
        window_height = 670
        window_width = 640
        ...
        config_parser["save_window_locations"]["general settings position"] != ""
        and config_parser["save_window_locations"]["general settings"] == "yes"
    ):
        general_settings_window.geometry(
            config_parser["save_window_locations"]["general settings position"]
        )
    ...
    path_frame.grid(
        column=0, row=0, columnspan=4, padx=5, pady=(10, 3), sticky=N + S + E + W
    )
    ...
    def set_mpv_path():
        path = filedialog.askopenfilename(
            title='Select Location to "mpv.exe"',
    ...
    # Media Info Gui Path ---------------------------------------------------------------------------------------------
    mediainfogui_frame = LabelFrame(
        path_frame, text=" MediaInfo GUI ", labelanchor="nw"
    )
    mediainfogui_frame.grid(
        column=0, row=2, columnspan=4, padx=5, pady=(5, 3), sticky=E + W
    )
    mediainfogui_frame.configure(
        fg="#3498db", bg="#434547", bd=3, font=(set_font, 9, "italic")
    )
    ...
    def set_mediainfogui_path():
        ...
        if path:
            ...
            mediainfogui_entry_box.insert(
                ...
                str(
                    pathlib.Path(
                        str(mediainfo_cfg_parser["mediainfogui_path"]["path"]).replace(
                            '"', ""
                        )
                    ).resolve()
                ),
    ...
    set_mediainfogui_path = HoverButton(
        mediainfogui_frame,
        text="Set Path",
    ...
    mediainfogui_entry_box = Entry(
        mediainfogui_frame, borderwidth=4, background="#CACACA"
    )
    mediainfogui_entry_box.grid(
        row=0, column=1, columnspan=3, padx=5, pady=5, sticky=N + S + E + W
    )
    ...
    def set_qaac_path():
        ...
        if path:
            ...
            qt_folder_path = pathlib.Path(
                pathlib.Path(str(qaac_parser["qaac_path"]["path"]).replace('"', ""))
                .resolve()
                .parent
                / "QTfiles64"
            )
    ...
    def update_qaac_file_paths():
        ...
        if update_parser["qaac_path"]["path"] == "":  # if path to qaac is empty
            ...
            qt_folder = pathlib.Path(
                pathlib.Path(str(update_parser["qaac_path"]["path"]).replace('"', ""))
                .resolve()
                .parent
                / "QTfiles64"
            )
    ...
    qaac_status_label = Label(
        qaac_frame, text="", background="#434547", foreground="white", width=10
    )
    qaac_status_label.grid(
        row=1, column=0, columnspan=2, padx=(5, 5), pady=(2, 0), sticky=W + E
    )
    ...
    qt_files_download.grid(
        row=1, column=2, columnspan=1, padx=5, pady=(2, 0), sticky=N + S + E + W
    )

    qt_files_dir = HoverButton(
        qaac_frame,
    ...
    qt_files_dir.grid(
        row=1, column=3, columnspan=1, padx=5, pady=(2, 0), sticky=N + S + E + W
    )

    # save path frame -------------------------------------------------------------------------------------------------
    save_path_frame = LabelFrame(settings_tab, text=" Save Paths ", labelanchor="n")
    save_path_frame.grid(
        column=0, row=1, columnspan=4, padx=5, pady=(10, 3), sticky=N + S + E + W
    )
    save_path_frame.configure(
    ...
    # Manual Auto Path ------------------------------------------------------------------------------------------------
    manual_auto_frame = LabelFrame(
        save_path_frame, text=" Manual/Auto Encode Path ", labelanchor="nw"
    )
    manual_auto_frame.grid(
        column=0, row=0, columnspan=4, padx=5, pady=(5, 3), sticky=E + W
    )
    manual_auto_frame.configure(
        fg="#3498db", bg="#434547", bd=3, font=(set_font, 9, "italic")
    )
    ...
    def set_manual_auto_path():
        path = filedialog.askdirectory(
            title="Output Path Manual/Auto", parent=general_settings_window
        )
        if path:
            ...
            manual_auto_entry_box.insert(
                0, str(pathlib.Path(str(manual_path_parser["output_path"]["path"])))
            )
            manual_auto_entry_box.config(state=DISABLED)

    set_manual_auto_path = HoverButton(
        manual_auto_frame,
        text="Set Path",
    ...
    manual_auto_entry_box = Entry(
        manual_auto_frame, borderwidth=4, background="#CACACA"
    )
    manual_auto_entry_box.grid(
    ...
    def reset_manual_auto_path():
        ...
        if msg:
            ...
            manual_auto_entry_box.delete(0, END)
            manual_auto_entry_box.insert(
                0, str(reset_manual_path_parser["output_path"]["path"]).title()
            )
            manual_auto_entry_box.config(state=DISABLED)

    reset_manual_auto_path = HoverButton(
        manual_auto_frame,
        text="X",
    ...
    reset_manual_auto_path.grid(
        row=0, column=3, columnspan=1, padx=5, pady=5, sticky=N + S + E + W
    )

    # Batch Path ------------------------------------------------------------------------------------------------------
    batch_frame = LabelFrame(
        save_path_frame, text=" Batch Processing Path ", labelanchor="nw"
    )
    ...
    def set_batch_path():
        ...
        if path:
            ...
            batch_entry_box.insert(
                0, str(pathlib.Path(str(bath_path_parser["batch_path"]["path"])))
            )
    ...
    def reset_batch_path():
        ...
        if msg:
            ...
            batch_entry_box.delete(0, END)
            batch_entry_box.insert(
                0, str(reset_batch_path_parser["batch_path"]["path"]).title()
            )
            batch_entry_box.config(state=DISABLED)
