...
root = Tk()
root.title("FFMPEG Audio Encoder v1.2")
...
def openaboutwindow():
    ...
    about_window_text.insert(INSERT, "FFMPEG Audio Encoder 1.2 \n")
...
def openaudiowindow():
    ...
    def show_streams_ffprobe(): # Stream Viewer 
        commands = ffprobe + " -v error -select_streams a -show_entries stream=index,codec_name,codec_type,channels,channel_layout,bit_rate " + VideoInputQuoted
        run = subprocess.Popen(commands, creationflags=subprocess.CREATE_NO_WINDOW, universal_newlines=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                               stdin=subprocess.PIPE)
        stream_window = Toplevel(audio_window)
        stream_window.title("Audio Streams")
        stream_window.configure(background="#434547")
        Label(stream_window, text="---- Audio Streams ----\n Index is equal to Track #", font = ("Times New Roman", 16), background='#434547', foreground="white").grid(column=0, row=0)
        text_area = scrolledtextwidget.ScrolledText(stream_window, width=40, height=25)
        text_area.grid(column=0, pady=10, padx=10)
        text_area.insert(INSERT, run.communicate())
        text_area.configure(font=("Helvetica" ,15))
    ...
    if encoder.get() == "AC3":
        ...
        my_menu.add_command(label="View Streams", command=show_streams_ffprobe)
...
def startaudiojob():
    ...
    if encoder.get() == "AC3":
        if shell_options.get() == "Default":
            ...
            subprocess.Popen('cmd /k ' + commands)
        ...
        if shell_options.get() == "Default":
            ...
            commands = '"' + ffmpeg + " -analyzeduration 100M -probesize 50M -i " + VideoInputQuoted + acodec_stream_choices[acodec_stream.get()] + dts_settings_choices[dts_settings.get()] + " -sn -vn -map_chapters -1 " + VideoOutputQuoted + " -hide_banner" + '"'
            subprocess.Popen('cmd /k ' + commands)
